...
for (const name of ["DEVELOPER_ID", "APPLE_TEAM_ID", "SIGNING_KEYCHAIN", "NOTARY_KEY", "NOTARY_KEY_ID", "NOTARY_ISSUER", "APP_VERSION"]) delete env[name];
...
const secretNames = ["DEVELOPER_ID", "APPLE_TEAM_ID", "SIGNING_CERTIFICATE", "SIGNING_PASSWORD", "NOTARY_PRIVATE_KEY", "NOTARY_KEY_ID", "NOTARY_ISSUER"];
...
assert.deepEqual(Object.keys(job.steps.find(step => step.run === "scripts/sign-and-package.sh").env).sort(), ["DEVELOPER_ID", "APPLE_TEAM_ID", "NOTARY_KEY_ID", "NOTARY_ISSUER"].sort());
assert.equal(release.jobs.homebrew.needs, "release");
assert.equal(release.jobs.homebrew.if, "vars.ENABLE_HOMEBREW == 'true'");
assert.ok(release.jobs.homebrew.steps.find(s => s.name === "Update tap after release exists").env.TAP_TOKEN);
assert.doesNotMatch(JSON.stringify(job), /TAP_TOKEN/);
assert.ok("workflow_dispatch" in release.on);
assert.match(byName("Require all release credentials and valid tag").run, /GITHUB_REF_TYPE.*tag/);
assert.equal(byName("Publish immutable release artifacts").if, undefined, "publication requires success of preceding signing and mounted verification steps");
const publish = byName("Publish immutable release artifacts").run;
assert.match(publish, /--jq .immutable/);
assert.doesNotMatch(JSON.stringify(job), /immutable-releases/);
assert.doesNotMatch(byName("Import temporary signing credentials").run, /list-keychains/);
assert.equal(byName("Remove credentials").if, "always()");
assert.equal(byName("Preserve notarization reports").if, "always()");
